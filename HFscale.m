function [scaledfids, sctype] = HFscale(traj,fids,sctype,Method_Params)
...
switch sctype
    ...
    case 'HF'
        ...
        % distance matrix
        rad = sqrt((traj1(:,:,1)-k0).^2+(traj1(:,:,2)-k0).^2+(traj1(:,:,3)-k0).^2);
        selmat = rad>dfromk0;
        scaledfids = fids.*selmat.*scale_factor + fids.*(1-selmat);
    case 'first'
