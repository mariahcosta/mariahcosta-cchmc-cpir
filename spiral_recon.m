...
%% Deal with Retrospective Gating - Need to make this better - get code from Alex
        ...
        for j = 1:size(FIDMat,4)
            for i = 1:size(FIDMat,3)
                ...
                %% MC 2D k0 scale
                tmp_FIDMat = FIDMat(:,:,i,j);
                [n_fid, sctype] = HFscale_2D(traj_recon,tmp_FID,'all',Method_Params,FIDs,tmp_FIDMat);
                %%
  disp('scaled------------------------------')
                % MC TAG 12/13 - uncomment?
%                tmp_FID_test = reshape(FIDMat(:,:,i,j),1,[])';
% 
%                                 if (Config_Params.DiscardFromBeginning ~= 0) || (Config_Params.DiscardFromEnd ~= 0)
%                     find_zeros = find(isnan(tmp_FID));
%                     tmp_FID(find_zeros) = [];
...
%                 [Norm_k0_Images(:,:,:,i,j),~,~] = base_noncart_recon((ImSize),nfid_C(1:size(traj_recon,1),1),traj_recon);
                 [Norm_k0_Images(:,:,:,i,j),~,~] = base_noncart_recon(ImSize,nfid_C,traj_recon);
               if i ==6
                 figure;plot(abs(nfid_C));title('norm')
                 figure;plot(abs(tmp_FID));title('original')
                 figure;plot(abs(tmp_FID)-abs(nfid_C));title('original - norm')

               end
%                 imslice(abs(squeeze(Norm_k0_Images)))
...
%                                         if Config_Params.VentBPM ~=0  && ~fast_recon
                                ...
                                %with the easy cases:
                                %tmp_FID = reshape(FIDMat(:,:,i,j),1,[])';
                                % open BA 24Sep2021 - fix matrix size for raw scaling
                                % the next two lines overwrite the traj and fid that were trimmed a few
                                % lines above...do I need to duplicate the commands above or just pass the
                                % trimmed traj and fid through to this section??
                                %                 tmp_FID = squeeze(FIDMat(:,:,i,j)); % i need to look to
                                %                 see if this was part of the og code
                                tmp_fid = tmp_FID;

                                % open BA 09Sep2021 - add in k-space scaling function
                                 [n_fid, sctype] = HFscale(traj_recon,tmp_fid,'HF',Method_Params);


                                % MC change 10/28/21
                                % [n_fid, sctype] = HFscale(traj_recon,tmp_fid,'all',Method_Params);


%                                 [n_fid, sctype] = HFscale_2D(traj_recon,tmp_fid,'all',Method_Params,FIDs);

                                % close BA 09Sep2021 - add in k-space scaling function
...
imslice(abs(squeeze(Norm_k0_Images)),'Norm_k0_Images')    
imslice(abs(squeeze(Images)),'Original Images')         
imslice(abs(squeeze(Images))-abs(squeeze(Norm_k0_Images)),'Original - Normalized');colormap(Cmap)
           
Recon.Keys = KeyImages;
Recon.FA_Map = FA_Map;
Recon.M0 = M0;
Recon.Images = Images;
% MC TAG 12/13 might need to untag
...
if exist('sctype') == 1
    ...
    Recon_Params.kspace_scale_type = "n/a";
end
